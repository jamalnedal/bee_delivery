...
//الطلبات الراجعة لموظف التوصيل
class ReturnOrderMan extends StatefulWidget {
  const ReturnOrderMan({Key? key}) : super(key: key);

  @override
  _ReturnOrderManState createState() => _ReturnOrderManState();
}

class _ReturnOrderManState extends State<ReturnOrderMan> with Helper  {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
          stream: FbFirestoreReturnsOrdersController().readCustomerMenReturnedOrders(),
          builder: (context, snapshot) {
            //كود لقراءة واستدعاء جميع الطلبات الراجعة من قبل الموظف
            if (snapshot.connectionState == ConnectionState.waiting) {
              return const Center(
                child: const CircularProgressIndicator(),
              );
            } else if (snapshot.hasData && snapshot.data!.docs.isNotEmpty) {
              List<QueryDocumentSnapshot> documentReturnOrder = snapshot.data!.docs;
              return
                ListView.builder(
                  scrollDirection: Axis.vertical,
                  itemCount: documentReturnOrder.length,
                              ...
                              child: ListTile(
                                onTap: () {
                                  Navigator.push(context, MaterialPageRoute(builder: (
                                      context) => OrderDescription(orderDescription: manReturnOrderInformation(documentReturnOrder[index]))));
                                },
                                  leading: const Icon(
                                      Icons.keyboard_return),
                                  subtitle: Text(documentReturnOrder[index].get('location'),
                                  ...
                                  title:
                                  Text(documentReturnOrder[index].get('firstName'),style:const TextStyle(
                                      fontFamily: 'Tajawal',
                                      fontWeight: FontWeight.bold,
                                      color: Colors.black),),
                                  trailing:  Text(documentReturnOrder[index].get('orderStatus'),style: const TextStyle(
                                      fontFamily: 'Tajawal',
                                      fontWeight: FontWeight.bold,
                                      color:Colors.red),)
                              ),
  ...
  Orders manReturnOrderInformation(documentSnapshot) {
    Orders order = Orders();
